...
- [ ] **1.6 — Create API routes** — New file `src/api/mindmap_routes.py`. Router at prefix `/api/v1/mindmaps` (handled in `main.py` registration, not in-file). Endpoints:
  ...
  Use `get_db()` dependency from `src.main`. Return JSON responses with consistent error shape: `{"error": "error_code", "message": "...", "detail": {}}`. (covers: AC-2, AC-3, AC-8, AC-9)
...
- [ ] **1.9 — Create frontend API client** — New file `tauri-app/src/features/mindmap/api.ts`. Functions following existing `client.ts` patterns (axios to `http://localhost:8000/api/v1/mindmaps`):
