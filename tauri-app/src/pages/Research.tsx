      ...
      <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
        ...
        {!active ? (
          ...
          <Card className="overflow-hidden">
            ...
            {/* Deductive audit — contradictions no single-claim check can catch,
                because each track was generated by a separate model call. */}
            {active.audit && active.audit.conflicts.length > 0 && (
              <div className="border-b border-warning/30 bg-warning/5 px-5 py-3">
                <div className="flex items-start gap-2">
                  <IconAlert size={15} className="mt-0.5 shrink-0 text-warning" />
                  <div className="min-w-0 flex-1">
                    <div className="text-[13px] font-medium text-ink">
                      {active.audit.conflicts.length} contradiction
                      {active.audit.conflicts.length === 1 ? '' : 's'} between findings
                    </div>
                    <p className="mt-0.5 text-[11px] text-ink-muted">
                      Machine-checked across all {active.audit.checked} findings
                      {active.audit.engine !== 'local' && ` · ${active.audit.engine}`}.
                      Research tracks are generated independently, so they can disagree.
                    </p>
                    <ul className="mt-2 space-y-2">
                      {active.audit.conflicts.map((c, i) => (
                        <li key={i} className="rounded-xl border border-line bg-surface px-3 py-2">
                          <p className="text-[11px] font-medium text-warning">
                            {c.explanation}
                          </p>
                          <p className="mt-1 text-[12px] text-ink">
                            {c.left_track && (
                              <span className="text-ink-muted">{c.left_track}: </span>
                            )}
                            {c.left}
                          </p>
                          <p className="mt-1 text-[12px] text-ink">
                            {c.right_track && (
                              <span className="text-ink-muted">{c.right_track}: </span>
                            )}
                            {c.right}
                          </p>
                        </li>
            ...
            {active.audit && active.audit.consistent && active.audit.checked > 1 && (
              <div className="flex items-center gap-1.5 border-b border-line bg-success/5 px-5 py-2 text-[11px] text-ink-muted">
                <IconCheck size={13} className="text-success" />
                No contradictions found across {active.audit.checked} findings.
              </div>
            )}
