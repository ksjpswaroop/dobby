...
def queries_prompt(topic: str, track: str, plan: str, count: int = 3) -> str:
    ...
    return (
        ...
        "- **Every query must be self-contained and unambiguous on its own.** A "
        "search engine sees only the query, not this conversation. Carry the "
        "distinguishing words of the subject into each one.\n"
        f"  Bad: `Industry` — returns a TV show and restaurants.\n"
        f"  Good: `{topic} market size` — unmistakably about the subject.\n"
        "- Each query must be different from the others in what it would return.\n"
        "- 4 to 10 words. Keywords, not full-sentence questions.\n"
